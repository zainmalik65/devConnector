...
router.put('like/:id',auth,async(req,res)=>{
    try {
       ...
       if(post.likes.filter(like=>like.user.toString()===req.user.id).lenght>0){
        return req.status(400).json({msg:'Post already Liked'});
       }
       post.likes.unshift({user:req.user.id});
...
router.put('unlike/:id',auth,async(req,res)=>{
    try {
       ...
       //check if the post has already been liked
       if(post.likes.some((like)=>like.user.toString()===req.user.id).lenght===0){
        return req.status(400).json({msg:'Post has not yet been Liked'});
       }
       //Get remove index
